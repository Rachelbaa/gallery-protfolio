...
var gBooks = getBooks();
// console.log("🚀 ~ file: book.servies.js ~ line 6 ~ gBooks", gBooks)
const KEY = 'booksAPI'
...
function handleBooks(books, bookTitle) {
  let booksList = [];
  for (let i = 0; i < books.length; i++) {
    let book = books[i];
    // booksList.push(book)
    // console.log('text 1: title: ', book.volumeInfo.title , '  ID :' ,book.id);
    if (book.volumeInfo.title.toLowerCase().substr(0, bookTitle.length) === bookTitle.toLowerCase()) {
      // console.log('text 222222222222: title: ', book.volumeInfo.title , '  ID :' ,book.id);
      let indexBook = booksList.findIndex(object => {
        return object.volumeInfo.title === book.volumeInfo.title;
      })
      // console.log('indexxxxxxxxxxxxxxxxxx ', book.volumeInfo.title, indexBook);
      if (indexBook === -1) {
        booksList.push(book)
      }
    }
...
// book.volumeInfo.title.toLowerCase().substr(0, bookTitle.length) === bookTitle.toLowerCase()
// book.volumeInfo.title.toLowerCase().includes(bookTitle.toLowerCase())

function getBooksFromApi(bookTitle) {
  return new Promise((resolve, reject) => {
    fetch(`https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%3javascript`)
      .then((res) => { return res.json(); })
      .then((result) => {
        Utils.saveToStorage(KEY, result.items);
        console.log('save to storage API:', result.items);
...
function getBooksFromTitle(bookTitle) {
  return new Promise((resolve,reject)=> {
    let books = Utils.loadFromStorage(KEY);
    if (!books) {
      books = getBooksFromApi(bookTitle);
      books.then(res => {
        console.log('test 3: ', res);
        const booksList = handleBooks(res, bookTitle)
        resolve(booksList)
      })
    }else{
      const booksList = handleBooks(books, bookTitle)
      resolve(booksList)
    }
...
function saveGBooks() {
  Utils.saveToStorage('books', gBooks)
}
...
function getByIdApi(bookId) {
  const books = Utils.loadFromStorage(KEY);
...
function addReview(bookId, review) {
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    if (book.id === bookId) {
      ...
      console.log('test 3:', books[i], i);
      book.reviews.push(review)
      Utils.saveToStorage('books', books)
      console.log('savwd review!!!', book.id);
    }

  }
}

function addBookToGBooks(book) {
  console.log( 'boookkkkkkssss-before: ', gBooks);
  gBooks.unshift(book)
  saveGBooks()
  console.log( 'boookkkkkkssss-after: ', gBooks);
}
